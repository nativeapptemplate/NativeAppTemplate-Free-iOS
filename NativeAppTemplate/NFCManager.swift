...
extension NFCManager: NFCNDEFReaderSessionDelegate {
    ...
    func readerSession(_ session: NFCNDEFReaderSession, didDetect tags: [NFCNDEFTag]) {
        ...
        guard let nfcTag = tags.first else { return }
        nonisolated(unsafe) let tag = nfcTag
                ...
                case .readWrite:
                    ...
                    case .write:
                        if capacity < self.userNdefMessage!.length {
                            let errorMessage = "Tag capacity is too small. " +
                                "Minimum size requirement is \(self.userNdefMessage!.length) bytes."
    ...
    private func write(session: NFCNDEFReaderSession, tag: NFCNDEFTag) {
        guard let userNdefMessage else { return }
