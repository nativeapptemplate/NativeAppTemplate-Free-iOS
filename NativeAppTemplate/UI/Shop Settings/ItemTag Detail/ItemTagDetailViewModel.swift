...
final class ItemTagDetailViewModel {
    ...
    func saveImageToPhotoAlbum() {
        guard let customerTagQrCodeImage else { return }
        ...
        getSaveToPhotoAlbumPermissionIfNeeded { granted in
            ...
            self.imageSaver.save(image: customerTagQrCodeImage) { error in
                if let error {
                    ...
                    self.messageBus.post(message: Message(
                        level: .success,
                        message: .customerQrCodeImageSavedToPhotoAlbum
                    ))
                }
            }
        }
    ...
    private func createNdefMessage(itemTag: ItemTag, itemTagType: ItemTagType) -> NFCNDEFMessage {
        ...
        let androidAarPayload = NFCNDEFPayload(
            format: .nfcExternal,
            type: Data(String.androidAarNfcndefPayloadType.utf8),
            identifier: Data(),
            payload: androidAarPayloadData
        )

        return if itemTagType == ItemTagType.server {
